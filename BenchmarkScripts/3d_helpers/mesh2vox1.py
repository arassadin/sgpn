    ...
    for instance in instances:
        ...
        for ind, vertex in enumerate(mesh_vertices):
            if instance['vertices'][ind] == '0\n':
                ...
                if grid_coord[1] < 48 and blob['data'][0][int(grid_coord[0]),int(grid_coord[1]), int(grid_coord[2])] <= 1.0:
                    counter += 1
        ...
        for grid_coord in grid_coords:
            mask[int(grid_coord[0]-minx), int(grid_coord[1]-miny), int(grid_coord[2]-minz)] = 1
...
parser.add_argument('--output_dir', default="/mnt/raid/ji/SGPN/SGPN_merged1", help='output file')
